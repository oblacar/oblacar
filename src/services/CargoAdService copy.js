...
const getAll = async () => {
  ...
  const updates = []; // накопим промисы перезаписей
  snap.forEach((childSnap) => {
    const key = childSnap.key;
    const raw = childSnap.val();

    const { ad: fixed, changed } = migrateOwnerInSnapshot(raw);

    if (changed) {
      // перезапишем «починенную» версию
      const adRef = child(cargoAdsRef, key);
      updates.push(update(adRef, fixed));
    }

    result.push({ adId: key, ...fixed });
  });

  // дождёмся всех апдейтов (без падения всего чтения при единичной ошибке)
...
const updateById = async (adId, patch = {}) => {
  ...
  // нормализуем автора + добавим updatedAt
  const payload = normalizeForDb({
    ...patch,
...
// 1) Нормализация ПЕРЕД ЗАПИСЬЮ (create/update): собираем snapshot автора и чистим легаси-ключи
function normalizeOwnerForWrite(payload = {}) {
  const p = { ...payload };

  // соберём owner.* из возможных легаси полей
  ...
  // подчистим легаси-ключи в БД: в update() null = удалить поле
  p.ownerName = null;
...
// 2) Миграция ПОСЛЕ ЧТЕНИЯ (getAll/getById): если нашли легаси — вернём "починенный" объект и флаг changed
function migrateOwnerInSnapshot(raw = {}) {
  ...
  // перенесём легаси в owner, не затирая уже существующие owner.*
  ...
  // если легаси-поля присутствовали — удалим их
...
// === ДОП. общая нормализация перед записью (оставь свою как было) ===
function normalizeForDb(ad = {}) {
  let copy = { ...ad };

  // встроим нормализацию автора
  copy = normalizeOwnerForWrite(copy);

  // пример: если используешь preferredLoadingTypes как массив → храним объектом (по желанию)
  ...
  // оставь остальное, что у тебя уже было (photo urls и т.п.)
  return copy;
}
