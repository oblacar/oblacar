...
export default function CargoListToolbar({
    ...
    sort = 'price_desc',
    onSortChange = () => { },
    // { cargoTypes:[], loadTypes:[], packaging:[], features:[] }
    filters: externalFilters = null,
    onFiltersChange = () => { },
}) {
    const [filters, setFilters] = React.useState(
        externalFilters ?? { cargoTypes: [], loadTypes: [], packaging: [], features: [] }
    );

    React.useEffect(() => {
        if (externalFilters) {
            setFilters(prev => ({
                cargoTypes: externalFilters.cargoTypes ?? prev.cargoTypes ?? [],
                loadTypes: externalFilters.loadTypes ?? prev.loadTypes ?? [],
                packaging: externalFilters.packaging ?? prev.packaging ?? [],
                features: externalFilters.features ?? prev.features ?? [],
            }));
        }
    }, [externalFilters]);
        ...
        <div className={`cargo-toolbar ${className}`}>
            <div className="ctb-bar">
                ...
                <div className="ctb-left">
                    ...
                    {/* ВАЖНО: используем loadTypes (совпадает с filterCargoAds) */}
                    <MultiCheckDropdown
                        label="Тип загрузки"
