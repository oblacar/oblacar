...
async function fetchAllIfExists(path) {
  ...
  const obj = snap.val();
  // кладём ключ как id (и НЕ даём данным его перезаписать)
  return Object.keys(obj).map((key) => ({ ...obj[key], id: key, _root: path }));
}
...
function filterType(rows, type) {
  ...
  return rows.filter((r) => r.type === type); // 'cargo' | 'transport'
}
